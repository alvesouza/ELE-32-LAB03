...
while i<lim
    ...
    parfor j = i : h1
        %i = j;
        pViterbiEuclid1(j) = perro(linhas,bits,Ei_N0(j),q,transicoes1, saidas1,1);
        %pViterbiEuclid2(j) = perro(linhas,bits,Ei_N0(j),q,transicoes2, saidas2,1)
        %pViterbiEuclid3(j) = perro(linhas,bits,Ei_N0(j),q,transicoes3, saidas3,1)
    end
    i = h1;
end
...
plot(20*log(Ei_N0)/log(10),log(pViterbiEuclid1)/log(10));
%plot(p2,pb2);
%plot(p3,pb3);
hold off