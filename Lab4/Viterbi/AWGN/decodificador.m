...
for i = 1:size(custoAdcional1,2)
    for j = 1:size(custoAdcional1,1)
        custoAdcional1(j,i) = pdist([bitsSaida1(j,:);MatrixVlinha(:,i)'],'euclidean')^2;
        custoAdcional2(j,i) = pdist([bitsSaida2(j,:);MatrixVlinha(:,i)'],'euclidean')^2;
    end
end
while indexColuna > 0
    if indexColuna <= fim
